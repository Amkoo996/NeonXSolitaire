...
export const Card = ({ card, onClick, isClickable = true, className, style, drag, onDragEnd, dragConstraints, playerColor = '#3b82f6' }: CardProps) => {
  const getSuitIcon = (size: string = "w-full h-full") => {
    const suitColor = (card.suit === 'hearts' || card.suit === 'diamonds') ? "text-red-600 fill-red-600" : "text-black fill-black";
  ...
  if (!card.isFaceUp) {
    return (
      <div 
        className={cn(
          "w-[50px] h-[68px] sm:w-[58px] sm:h-[82px] md:w-[66px] md:h-[94px] lg:w-[72px] lg:h-[102px] rounded-lg md:rounded-2xl bg-white border-2 shadow-md relative overflow-hidden group transition-all",
          className
        )
      }
        style={{ borderColor: `${playerColor}44`, boxShadow: `0 4px 10px ${playerColor}22` }}
      >
        <div className="absolute inset-0 bg-gradient-to-br from-white via-slate-50 to-slate-100" />
        <div 
          className="absolute inset-1.5 md:inset-2 border-2 border-dashed rounded-md md:rounded-xl flex items-center justify-center opacity-60"
          style={{ borderColor: playerColor, backgroundColor: `${playerColor}08` }}
        >
           <div 
             className="font-black text-[10px] md:text-xl select-none tracking-widest"
             style={{ color: playerColor }}
           >
             NEON
           </div>
        </div>
        {/* Subtle pattern for back of card */}
        <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: `radial-gradient(${playerColor} 1px, transparent 1px)`, backgroundSize: '8px 8px' }} />
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start leading-none z-20 relative p-1 md:p-2">
        <span className={cn(
          "font-black italic tracking-tighter leading-none mb-1", 
          card.rank.length > 1 ? "text-[11px] md:text-2xl" : "text-[14px] md:text-3xl",
          getTextColor()
        )}>
          {card.rank}
        </span>
      ...
      {/* Decorative corner suit for realism but kept clean */}
      <div className="absolute bottom-2 right-2 opacity-10 pointer-events-none rotate-12">
